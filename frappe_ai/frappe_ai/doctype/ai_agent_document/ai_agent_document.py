...
from frappe_ai.rag.vector_store import add_or_update_document, delete_document
...
class AIAgentDocument(Document):
    ...
    def on_trash(self):
        try:
            ...
            frappe.log_error(
                title="Frappe AI: failed to remove document from vector store",
    ...
    def _sync_to_vector_store(self):
        try:
            ...
            frappe.log_error(
                title="Frappe AI: failed to embed document",
                message=frappe.get_traceback(),
            )
