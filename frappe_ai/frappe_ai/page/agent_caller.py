...
the Desk, not hardcoded here:
  - Functions: `Agent Function` doctype (name, description, parameter
    schema, target DocType, and a dotted-path dispatch handler).
...
`run()` performs the actual creation, and is only ever called after the
user has reviewed (and can edit) the proposal in the page.
"""
...
def _get_functions() -> dict:
    ...
    rows = frappe.get_all(
        "Agent Function",
        filters={"enabled": 1},
        fields=["function_name", "description", "parameters", "target_doctype", "dispatch_handler"],
    )
    functions = {}
    for row in rows:
        try:
        ...
        functions[row["function_name"]] = {
            ...
            "dispatch_handler": row["dispatch_handler"],
        }
...
def run(name: str, arguments):
    """Actually create the document. Only call this after the user has reviewed plan()'s output."""
...
# ---------------------------------------------------------------------------
# Built-in dispatch handlers (referenced by dotted path from Agent Function
# records -- see fixtures/agent_function.json). Add your own functions here,
# or in any other importable module, and point a new Agent Function record's
# Dispatch Handler at them.
# ---------------------------------------------------------------------------
    ...
    doc = frappe.get_doc(
        {
            ...
            "items": [
                {"item_code": item["item_name"], "qty": item.get("qty", 1)}
                for item in items
            ],
        }
    )
