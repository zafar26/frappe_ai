...
frappe_ai.AgentCaller = class AgentCaller {
		...
		this.$wrapper = $(`	
			<div style="padding: 20px;">
				<div class="agent-caller">
					...
					<strong>Approve &amp; Run</strong>. If a step needs something
	...
	/**
	 * Render one reviewable step: an editable JSON box for {name, arguments},
	 * with Approve & Run / Discard actions. `note`, if given, explains why
	 * this step exists. `then`, if given, is queued as the next step once
	 * this one succeeds (e.g. retry the original Sales Order now that the
	 * Customer it needed has been created) -- still requires the user's
	 * approval like every other step, it's just queued up automatically.
	 */
	...
	handle_result(result, $step, $result, then) {
		...
		// Soft failure. Show the error, and if the script proposed a
		// follow-up step, surface that as a new reviewable step -- still
		// requires the user's approval, nothing auto-runs.
		$result.html(`
