...
from frappe_ai.rag.embeddings import EMBEDDING_DIM


class RouterClassifier(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int = 64, num_classes: int = 3):
        # input_dim has NO default on purpose -- it must always match
        # EMBEDDING_DIM from rag/embeddings.py. A previous version of this
        # code defaulted to 384 (the old MiniLM dimension), which silently
        # became wrong after upgrading to mpnet-base-v2 (768-dim): load_model()
        # below reconstructed the architecture without passing input_dim,
        # so it built a 384-input model and then tried to load 768-input
        # weights into it. Making this required prevents that class of bug
...
def load_model() -> Tuple[RouterClassifier, List[str]]:
    ...
    model = RouterClassifier(input_dim=EMBEDDING_DIM, num_classes=len(label_list))
