...
_complete_chat() is the shared low-level entry point (raw messages in,
raw text out) used both by the simple RAG path here AND by the agentic
tool-calling loop in agents/agentic_loop.py -- one backend dispatch,
reused by both call styles.
"""
...
def _complete_gguf(messages, max_new_tokens: int) -> str:
    llm = _get_gguf_llm()
...
def _complete_transformers(messages, max_new_tokens: int) -> str:
    settings = _get_settings()
...
# ---------------------------------------------------------------------------
# Shared low-level entry point -- raw messages in, raw text out.
# Used by both generate_response() below AND agents/agentic_loop.py.
# ---------------------------------------------------------------------------


def _complete_chat(messages, max_new_tokens: int = None) -> str:
    settings = _get_settings()
    max_new_tokens = max_new_tokens or settings.max_new_tokens or 256
    backend = settings.model_backend or "GGUF (llama.cpp)"

    if backend.startswith("GGUF"):
        return _complete_gguf(messages, max_new_tokens)
    return _complete_transformers(messages, max_new_tokens)


# ---------------------------------------------------------------------------
# Public entry point -- unchanged signature, simple single-shot RAG path
# ---------------------------------------------------------------------------
...
def generate_response(system_prompt: str, context_chunks: List[str], user_query: str) -> str:
    ...
    return _complete_chat(messages, max_new_tokens)
