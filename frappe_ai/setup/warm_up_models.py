...
def run():
    ...
    print(
        "\nDownloading/loading auto-categorization model (facebook/bart-large-mnli, "
        "~1.6GB -- this is the biggest one, only used to tag AI Agent Document "
        "metadata, not for chat)..."
    )
    from frappe_ai.rag.vector_store import _get_zero_shot_classifier
    _get_zero_shot_classifier()
    print(
        "Auto-categorization model ready. Saving an AI Agent Document will no "
        "longer trigger a cold model load."
    )
