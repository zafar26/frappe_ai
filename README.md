...
## Auto-categorization metadata (optional, not used for retrieval yet)

Every `AI Agent Document` gets tagged with an extra `auto_category` +
`confidence_score` in its Chroma metadata, predicted via zero-shot
classification (`facebook/bart-large-mnli`) against your current
`AI Agent` list. This is **not** used for routing or retrieval today --
it's stored for future filtering/search use cases. Categorization is
fail-safe: if the model can't load or classification errors for any
reason, the document still gets embedded and stored normally, just
without a category tag (`"unclassified"`).

The classifier model (~1.6GB) is cached as a singleton after first load
-- `warm_up_models.run` pre-loads it as part of setup, same predictable
pattern as the embedding model and LLM, so the first real document save
